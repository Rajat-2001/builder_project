...
import { addBonusHours, getFullHoursSummary, deleteBonusEntry } from "../api/bonus";
...
export default function AdminDashboard() {
  ...
  const [bonusForm,        setBonusForm]        = useState({ user_id:"", project_id:"", hours:"", reason:"" });
  const [bonusMsg,         setBonusMsg]         = useState("");
  const [bonusProjects,    setBonusProjects]    = useState([]);
  ...
  const loadHoursSummary = async () => {
    ...
    try {
      const [summary, projs] = await Promise.all([getFullHoursSummary(), getAllProjects()]);
      setHoursSummary(summary);
      setBonusProjects(projs);
    } catch (err) {
  ...
  const handleTabChange = (tab) => {
    ...
    if (tab === "invites"  && invites.length   === 0) loadInvites();
    if (tab === "projects" && projects.length  === 0) loadProjects();
    if (tab === "bonus"    && hoursSummary.length === 0) loadHoursSummary();
  };

  // ── Create user
  const handleCreateUser = async (e) => {
    e.preventDefault();
    setUserMsg(""); setCredentials(null);
    try {
      await createUser({ ...newUser, email: newUser.email || undefined });
      setCredentials({ full_name:newUser.full_name, phone:newUser.phone, password:newUser.password, role:newUser.role });
      setNewUser({ full_name:"", phone:"", password:"", role:"worker", email:"" });
      setShowAddUser(false);
      loadUsers();
  ...
  const handleToggleActive = async (userId, currentStatus) => {
    try { await toggleUserActive(userId, !currentStatus); loadUsers(); }
    catch (err) { setUserMsg(err.message); }
  };
  ...
  // ── Generate invite
  const handleGenerateInvite = async () => {
    if (!customerName.trim()) { setInviteMsg("Please enter the customer's name first."); return; }
    setInviteMsg(""); setGeneratedLink("");
    try {
      const data = await createInvite("customer");
      setGeneratedLink(`http://localhost:5173/join?token=${data.token}`);
      setInviteForName(customerName.trim());
      setCustomerName("");
      loadInvites();
  ...
  // ── Create project
  const handleCreateProject = async (e) => {
    e.preventDefault(); setProjectMsg("");
    try {
      await createProject({ ...newProject, start_date:newProject.start_date||undefined, end_date:newProject.end_date||undefined });
      setProjectMsg("✓ Project created successfully.");
      setNewProject({ name:"", description:"", start_date:"", end_date:"" });
      setShowAddProject(false);
      loadProjects();
    } catch (err) { setProjectMsg(err.message); }
  };

  // ── Add bonus hours
  const handleAddBonus = async (e) => {
    e.preventDefault(); setBonusMsg("");
    if (!bonusForm.user_id || !bonusForm.hours) { setBonusMsg("Please select a user and enter hours."); return; }
    try {
      const payload = {
        user_id: bonusForm.user_id,
        hours: parseFloat(bonusForm.hours),
        reason: bonusForm.reason || undefined,
        project_id: bonusForm.project_id || undefined,
      };
      const res = await addBonusHours(payload);
      setBonusMsg(`✓ ${res.hours}h bonus added to ${res.user}`);
      setBonusForm({ user_id:"", project_id:"", hours:"", reason:"" });
      setShowBonusForm(false);
      loadHoursSummary();
    } catch (err) { setBonusMsg(err.message); }
  };

  const filteredUsers = roleFilter === "all" ? users : users.filter(u => u.role === roleFilter);
  const staffUsers = users.filter(u => ["worker","team_lead","supervisor","architect"].includes(u.role));

  // ── If viewing site map, render SiteDashboard with a back button overlay
  if (viewingSite) {
    return (
      <div style={{ position:"relative" }}>
        ...
        <div style={{ position:"fixed", bottom:24, right:24, zIndex:999 }}>
          <button
            onClick={() => setViewingSite(false)}
            style={{ background:"#0A66C2", color:"#fff", border:"none", borderRadius:24, padding:"12px 22px", fontSize:14, fontWeight:700, cursor:"pointer", boxShadow:"0 4px 16px rgba(10,102,194,.4)", display:"flex", alignItems:"center", gap:8, fontFamily:"inherit" }}
          >
            ← Back to Admin
          </button>
        </div>
      ...
      <div style={s.navbar}>
        <div style={s.navInner}>
          <div style={s.navLogo}>
            ...
            <span style={s.navBrand}>BORG-1</span>
          </div>
          <div style={s.navRight}>
            <button
              onClick={() => setViewingSite(true)}
              style={{ padding:"8px 18px", background:"#EAF0F9", color:"#0A66C2", border:"2px solid #0A66C2", borderRadius:24, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit", transition:"all .15s" }}
            >
              🏗 View Site
            </button>
            <span style={s.navName}>{user?.full_name || user?.phone}</span>
            <span style={{...s.roleBadge, backgroundColor:ROLE_CONFIG.admin.bg, border:`1px solid ${ROLE_CONFIG.admin.border}`, color:ROLE_CONFIG.admin.color}}>Administrator</span>
            <button onClick={logout} style={s.logoutBtn} className="logout-hover">Sign out</button>
          </div>
        </div>
      ...
      <div style={s.main}>
        ...
        {activeTab === "users" && (
          <div>
            ...
            {showAddUser && (
              <div style={s.formCard}>
                <h3 style={s.formCardTitle}>Create New User</h3>
                <p style={{fontSize:"13px", color:"#666", marginBottom:"16px"}}>Set credentials below. After creation a credentials card appears — copy and share with the user.</p>
                <form onSubmit={handleCreateUser} style={s.formGrid}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Full name</label>
                    <input style={s.input} value={newUser.full_name} onChange={e => setNewUser({...newUser, full_name:e.target.value})} placeholder="John Smith" required/>
                  ...
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Role</label>
                    <select style={s.input} value={newUser.role} onChange={e => setNewUser({...newUser, role:e.target.value})}>
                      {DIRECT_ROLES.map(r => <option key={r} value={r}>{ROLE_CONFIG[r].label}</option>)}
                    </select>
                  </div>
                  <div style={{display:"flex", alignItems:"flex-end"}}>
                    <button type="submit" style={{...s.primaryBtn, width:"100%", padding:"12px"}}>Create User</button>
                ...
                {userMsg && !userMsg.startsWith("✓") && <div style={{...s.msgBox, ...s.msgError, marginTop:"12px"}}>{userMsg}</div>}
              </div>
            )}
            ...
            {credentials && (
              <div style={s.credCard}>
                ...
                <p style={{fontSize:"13px", color:"#666", marginBottom:"16px"}}>Share these credentials with <strong>{credentials.full_name}</strong>. Password won't be visible again after closing this.</p>
                <div style={s.credGrid}>
                  <div style={s.credItem}><span style={s.credLabel}>Name</span><span style={s.credValue}>{credentials.full_name}</span></div>
                  <div style={s.credItem}>
                    <span style={s.credLabel}>Role</span>
                    <span style={{fontSize:"12px", fontWeight:"600", padding:"3px 10px", borderRadius:"12px", backgroundColor:ROLE_CONFIG[credentials.role]?.bg, border:`1px solid ${ROLE_CONFIG[credentials.role]?.border}`, color:ROLE_CONFIG[credentials.role]?.color}}>
                      {ROLE_CONFIG[credentials.role]?.label}
                  ...
                  <div style={s.credItem}><span style={s.credLabel}>Phone (login ID)</span><span style={{...s.credValue, fontFamily:"monospace", fontSize:"16px", letterSpacing:"1px"}}>{credentials.phone}</span></div>
                  <div style={s.credItem}><span style={s.credLabel}>Password</span><span style={{...s.credValue, fontFamily:"monospace", fontSize:"16px", letterSpacing:"1px", color:"#0A66C2"}}>{credentials.password}</span></div>
                </div>
                <div style={{display:"flex", gap:"12px", marginTop:"16px", alignItems:"center"}}>
                  <button onClick={handleCopyCredentials} style={{...s.primaryBtn, display:"flex", alignItems:"center", gap:"6px"}}>
                    {credCopied ? "✓ Copied!" : "📋 Copy credentials"}
            ...
            <div style={s.filterRow}>
              {[["all","All"],["admin","Admin"],["team_lead","Team Leaders"],["worker","Workers"],["architect","Architects"],["supervisor","Supervisors"],["customer","Customers"]].map(([val, label]) => (
                <button key={val} onClick={() => setRoleFilter(val)} style={{...s.filterPill, ...(roleFilter===val?s.filterPillActive:{})}}>
                  {label}
                  <span style={{fontSize:"11px", marginLeft:"4px", opacity:0.7}}>
                    {val==="all" ? users.length : users.filter(u=>u.role===val).length}
                  </span>
                ...
                <table style={s.table}>
                  <thead>
                    <tr style={s.thead}>
                      ...
                      <th style={s.th}>Status</th>
                      <th style={s.th}>Joined</th>
                      {["worker","team_lead","architect","supervisor"].includes(roleFilter) && <th style={s.th}>Hours</th>}
                      <th style={s.th}>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredUsers.map((u, i) => {
                      ...
                      return (
                        <tr key={u.id} style={{...s.tr, ...(i%2===0?{}:s.trAlt)}}>
                          ...
                          <td style={s.td}>{u.email||"—"}</td>
                          <td style={s.td}>
                            <span style={{fontSize:"12px", fontWeight:"600", padding:"3px 10px", borderRadius:"12px", backgroundColor:rc.bg, border:`1px solid ${rc.border}`, color:rc.color}}>{rc.label}</span>
                          </td>
                          <td style={s.td}>
                            <span style={{fontSize:"12px", fontWeight:"600", padding:"3px 10px", borderRadius:"12px", backgroundColor:u.is_active?"#F0FAF5":"#FFF0F0", border:`1px solid ${u.is_active?"#B8DFC9":"#FFCCCC"}`, color:u.is_active?"#057642":"#CC1016"}}>
                              {u.is_active ? "Enabled" : "Disabled"}
                          ...
                          <td style={s.td}>{new Date(u.created_at).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"})}</td>
                          {["worker","team_lead","architect","supervisor"].includes(roleFilter) && (
                            <td style={s.td}><span style={{fontWeight:"600", color:"#0A66C2"}}>{hoursMap[u.id]||0}h</span></td>
                          )}
                          <td style={s.td}>
                            <div style={{display:"flex", gap:"8px"}}>
                              {u.role !== "admin" && (
                              ...
                              {u.role !== "admin" && (
                                <button onClick={() => handleDeleteUser(u.id, u.full_name)} style={{...s.actionBtn, ...s.actionBtnDanger}}>Delete</button>
                              )}
                            </div>
                          </td>
                        </tr>
        ...
        {activeTab === "invites" && (
          <div>
            <div style={s.sectionHeader}>
              <div>
                <h2 style={s.sectionTitle}>Customer Invites</h2>
                <p style={{fontSize:"13px", color:"#666", marginTop:"2px"}}>Generate a signup link for a customer.</p>
              </div>
            </div>
            <div style={s.formCard}>
              <h3 style={s.formCardTitle}>Generate Customer Invite Link</h3>
              <p style={{fontSize:"13px", color:"#666", marginBottom:"20px"}}>The link expires in <strong>48 hours</strong> and is <strong>single-use</strong>.</p>
              <div style={{display:"flex", gap:"12px", alignItems:"flex-end", flexWrap:"wrap"}}>
                <div style={{...s.fieldGroup, flex:1, minWidth:"200px"}}>
                  <label style={s.label}>Customer name <span style={{color:"#CC1016"}}>*</span></label>
                  <input style={s.input} value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="e.g. Smith Family"/>
                </div>
                <button onClick={handleGenerateInvite} style={{...s.primaryBtn, padding:"12px 28px"}}>Generate Link</button>
              </div>
              {generatedLink && (
                <div style={{marginTop:"20px"}}>
                  <label style={s.label}>Invite link for <span style={{color:"#0A66C2", fontWeight:"700"}}>{inviteForName}</span></label>
                  <div style={{display:"flex", gap:"8px", marginTop:"6px"}}>
                    <input readOnly value={generatedLink} style={{...s.input, flex:1, color:"#0A66C2", backgroundColor:"#EEF3FB"}}/>
                    <button onClick={handleCopy} style={{...s.primaryBtn, padding:"12px 20px"}}>{copied?"✓ Copied":"Copy"}</button>
                  </div>
                </div>
              )}
              {inviteMsg && <div style={{...s.msgBox, ...s.msgError, marginTop:"12px"}}>{inviteMsg}</div>}
        ...
        {activeTab === "projects" && (
          <div>
            ...
            {projectMsg && <div style={{...s.msgBox, ...(projectMsg.startsWith("✓")?s.msgSuccess:s.msgError)}}>{projectMsg}</div>}
            {projectsLoading ? <div style={s.loading}>Loading projects...</div> : projects.length === 0 ? (
              <div style={s.emptyState}>No projects yet.</div>
            ) : (
              <div style={s.projectsGrid}>
                {projects.map(p => (
                  <div key={p.id} style={s.projectCard}>
                    ...
                    <div style={{marginTop:"12px"}}>
                      ...
                      <div style={{height:"6px", backgroundColor:"#E0DFDC", borderRadius:"3px"}}>
                        <div style={{height:"100%", width:`${p.overall_progress}%`, backgroundColor:"#0A66C2", borderRadius:"3px"}}/>
                      </div>
                    </div>
                    {p.start_date && <div style={{marginTop:"10px", fontSize:"12px", color:"#666"}}>Started {p.start_date}</div>}
                  </div>
                ))}
              </div>
            )}
        ...
        {activeTab === "bonus" && (
          <div>
            <div style={s.sectionHeader}>
              <div>
                <h2 style={s.sectionTitle}>Bonus Hours</h2>
                <p style={{fontSize:"13px", color:"#666", marginTop:"2px"}}>Add bonus hours to any team member. These count toward their total hours.</p>
              </div>
              <button onClick={() => { setShowBonusForm(!showBonusForm); setBonusMsg(""); }} style={s.primaryBtn}>
                {showBonusForm ? "Cancel" : "+ Add Bonus Hours"}
              </button>
            </div>
            ...
            {showBonusForm && (
              <div style={s.formCard}>
                <h3 style={s.formCardTitle}>Add Bonus Hours</h3>
                <form onSubmit={handleAddBonus} style={s.formGrid}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Team member <span style={{color:"#CC1016"}}>*</span></label>
                    <select style={s.input} value={bonusForm.user_id} onChange={e => setBonusForm({...bonusForm, user_id:e.target.value})} required>
                      <option value="">Select a user...</option>
                      {staffUsers.map(u => (
                        <option key={u.id} value={u.id}>{u.full_name} ({ROLE_CONFIG[u.role]?.label})</option>
                      ))}
                    </select>
                  </div>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Project <span style={{color:"#999", fontWeight:"400"}}>(optional)</span></label>
                    <select style={s.input} value={bonusForm.project_id} onChange={e => setBonusForm({...bonusForm, project_id:e.target.value})}>
                  ...
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Bonus hours <span style={{color:"#CC1016"}}>*</span></label>
                    <input style={s.input} type="number" min="0.5" step="0.5" value={bonusForm.hours} onChange={e => setBonusForm({...bonusForm, hours:e.target.value})} placeholder="e.g. 2" required/>
                  </div>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Reason <span style={{color:"#999", fontWeight:"400"}}>(optional)</span></label>
                    <input style={s.input} value={bonusForm.reason} onChange={e => setBonusForm({...bonusForm, reason:e.target.value})} placeholder="e.g. Overtime on weekend"/>
                  </div>
                  <div style={{gridColumn:"1/-1"}}>
                    <button type="submit" style={{...s.primaryBtn, padding:"12px 32px"}}>Add Bonus Hours</button>
                  </div>
                </form>
                {bonusMsg && (
                  <div style={{...s.msgBox, ...(bonusMsg.startsWith("✓")?s.msgSuccess:s.msgError), marginTop:"12px"}}>{bonusMsg}</div>
                )}
              </div>
            )}

            {bonusMsg && !showBonusForm && (
              <div style={{...s.msgBox, ...(bonusMsg.startsWith("✓")?s.msgSuccess:s.msgError)}}>{bonusMsg}</div>
            )}

            {/* Hours summary table */}
            {summaryLoading ? (
              <div style={s.loading}>Loading hours summary...</div>
            ) : hoursSummary.length === 0 ? (
              <div style={s.emptyState}>No hours data yet.</div>
            ) : (
              <div style={s.tableWrap}>
                <table style={s.table}>
                  <thead>
                    <tr style={s.thead}>
                      <th style={s.th}>Name</th>
                      <th style={s.th}>Role</th>
                      <th style={s.th}>Real Hours</th>
                      <th style={s.th}>Bonus Hours</th>
                      <th style={s.th}>Total Hours</th>
                    </tr>
                  </thead>
                  <tbody>
                    {hoursSummary.map((r, i) => {
                      const rc = ROLE_CONFIG[r.role] || ROLE_CONFIG.worker;
                      return (
                        <tr key={r.user_id} style={{...s.tr, ...(i%2===0?{}:s.trAlt)}}>
                          <td style={s.td}>
                            <div style={s.userCell}>
                              <div style={{...s.userAvatar, backgroundColor:rc.bg, color:rc.color, border:`1px solid ${rc.border}`}}>
                                {(r.full_name||"U").charAt(0).toUpperCase()}
                              </div>
                              <span style={s.userName}>{r.full_name}</span>
                            </div>
                          </td>
                          <td style={s.td}>
                            <span style={{fontSize:"12px", fontWeight:"600", padding:"3px 10px", borderRadius:"12px", backgroundColor:rc.bg, border:`1px solid ${rc.border}`, color:rc.color}}>{rc.label}</span>
                          </td>
                          <td style={s.td}><span style={{fontWeight:"600", color:"#313335"}}>{r.real_hours}h</span></td>
                          <td style={s.td}>
                            <span style={{fontWeight:"600", color: r.bonus_hours > 0 ? "#057642" : "#aaa"}}>
                              {r.bonus_hours > 0 ? `+${r.bonus_hours}h` : "—"}
                            </span>
                          </td>
                          <td style={s.td}><span style={{fontWeight:"800", color:"#0A66C2", fontSize:"15px"}}>{r.total_hours}h</span></td>
                        </tr>
                      );
                    })}
                  </tbody>
...
const s = {
  ...
  credItem:     { display:"flex", flexDirection:"column", gap:"4px" },
  credLabel:    { fontSize:"11px", fontWeight:"600", color:"#999", textTransform:"uppercase", letterSpacing:"0.5px" },
  credValue:    { fontSize:"15px", fontWeight:"600", color:"#000000E6" },
  filterRow:    { display:"flex", gap:"8px", marginBottom:"16px", flexWrap:"wrap" },
  filterPill:   { padding:"6px 14px", borderRadius:"20px", fontSize:"13px", fontWeight:"600", cursor:"pointer", border:"1.5px solid #C9C5C0", backgroundColor:"#FFFFFF", color:"#666", fontFamily:"'Inter', sans-serif", transition:"all 0.15s" },
  filterPillActive: { backgroundColor:"#0A66C2", borderColor:"#0A66C2", color:"#FFFFFF" },
  tableWrap:    { backgroundColor:"#FFFFFF", borderRadius:"8px", boxShadow:"0 0 0 1px rgba(0,0,0,0.08)", overflow:"hidden" },
  ...
  projectCardName:   { fontSize:"15px", fontWeight:"700", color:"#000000E6" },
  footer:       { borderTop:"1px solid #E0DFDC", backgroundColor:"#FFFFFF", padding:"16px 24px", display:"flex", flexWrap:"wrap", gap:"16px", justifyContent:"center", marginTop:"auto" },
  footerItem:   { fontSize:"12px", color:"#666" },
};